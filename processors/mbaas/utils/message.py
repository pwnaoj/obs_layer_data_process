...
class PathAnalyzer:
    ...
    def _handle_list_navigation(current_event: list, key: str, path_so_far: List[str]) -> Tuple[Any, str, str]:
        ...
        return None, f"{key} not found in list items", list_projection_key
    ...
    def _handle_dict_navigation(current_event: dict, key: str) -> Tuple[Any, str]:
        ...
        return None, f"{key} not found in dict"
...
def get_type_at_each_level(event: dict = None, query: str = None) -> dict:
    ...
    for i, key in enumerate(keys):
        ...
        # Navegar según el tipo de estructura actual
...
def extract_from_message_selected_fields(selected_vars: List[list], event: dict) -> Generator[Tuple[str, Any], None, None]:
    """
    ...
    Args:
        selected_vars (List[list]): Lista con los queries a extraer y si están habilitados.
    ...
    """
    if not selected_vars or not event:
        return
    
    extractor = DataExtractor()
    
    try:
        for jmespath_var, enabled in selected_vars:
            ...
            result = extractor.extract_single_field(jmespath_var, event)
            yield (jmespath_var, result)
